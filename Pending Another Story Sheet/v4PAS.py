...
def process_orders(input_file, output_file):
    ...
    # Filter orders with 'Pending Another Story'
    pending_another_story = df[df[delivery_status_col] == 'Pending Another Story']

    # Dictionary to hold the merged data
    output_data = []

    # Process each unique Chat Reference for 'Pending Another Story'
    for chat_ref in pending_another_story[chat_ref_col].unique():
        ...
        # Get 'Pending Another Story' Orders
        pending_orders = group[group[delivery_status_col] == 'Pending Another Story']
        related_orders = group[group[delivery_status_col] != 'Pending Another Story']
        ...
        for _, row in related_orders.iterrows():
            output_data.append([
                ...
                row[package_col],
                row[country_col],
                row[conf_date].date() if pd.notnull(row[conf_date]) else None,  # Format Confirmation Date
                row[eta].date() if pd.notnull(row[eta]) else None               # Format ETA
            ])
    ...
    output_df = pd.DataFrame(output_data, columns=['Order IDs', 'Delivery Status', 'Phone', 'Department', 'Package', 'Country', 'Conf. Date', 'ETA'])
    ...
    for col in range(1, 9):  # Columns A to E
...
output_file = f'PAS {timestamp}.xlsx'
